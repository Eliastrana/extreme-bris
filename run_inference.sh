...
if dataset_complete "$MEPS"; then
  ...
  # Only the dates are pinned. The recipe's OPeNDAP URLs stay as
  # {date:strftime(...)} templates so each state is read from its OWN cycle,
  # as its own analysis - 18:00 from the 18Z file, t0 from the t0 file.
  #
  # That is only safe because scripts/anemoi_create.py repairs
  # iterate_patterns, which otherwise hands every resolved URL every date and
  # makes the two cycles collide. See the docstring there; the alternative,
  # pinning both states to one cycle, silently turns t0 into a +6 h forecast.
