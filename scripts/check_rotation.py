...
Two independent checks:
  ...
  * geostrophic balance — in the northern hemisphere the wind sits close to 90
    degrees anticlockwise from the pressure gradient. A systematic offset from
    that is what an unrotated field looks like.
...
def main() -> int:
    ...
    print("  northern-hemisphere geostrophic flow sits near +90;")
    print("  a systematic offset from that is an unrotated field.\n")
    ...
    verdict = abs(med - 90.0)
    if verdict < 25:
        print(f"VERDICT: consistent with a correctly rotated field ({verdict:.0f} deg from 90).")
        return 0
    print(f"VERDICT: {verdict:.0f} deg from the expected 90.")
    print("  Compare that with the rotation range above: if the discrepancy")
    print("  matches it, the rotation is missing or applied the wrong way.")
    return 2
